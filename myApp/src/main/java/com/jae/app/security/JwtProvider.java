...
public class JwtProvider {
	...
	public void init() {
		try {
			...
			InputStream resourceAsStream = getClass().getResourceAsStream("/springtaskapp.jks");
			keyStore.load(resourceAsStream, "secret".toCharArray());
		} catch (KeyStoreException | CertificateException | NoSuchAlgorithmException | IOException e) {
			throw new SpringTaskappException("Exception occurred while loading keystore");
		}
	}
	...
	private PrivateKey getPrivateKey() {
		try {
			return (PrivateKey) keyStore.getKey("springtaskapp", "secret".toCharArray());
		} catch (KeyStoreException | NoSuchAlgorithmException | UnrecoverableKeyException e) {
			throw new SpringTaskappException("Exception occured while retrieving public key from keystore");
		}
	}
	...
	private PublicKey getPublickey() {
		try {
			return keyStore.getCertificate("springtaskapp").getPublicKey();
